...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		File poem = new File("C:\\Users\\Keega\\Desktop\\School\\Software Development\\Poem.htm");
		...
		for (int i = 0; i < splitResults.length; i++) {
			if(splitResults[i].matches("[a-zA-Z]+")) {
				...
				if(match == true) {
					...
					//System.out.println("Increasing " + word.get(w) + " count to " + wordCount.get(w));
					match = false;
				}
				else {
					word.add(temp);
					wordCount.add(1);
					//System.out.println("Adding " + temp + " to word list");
				}
				w = 0;
			}
		}
